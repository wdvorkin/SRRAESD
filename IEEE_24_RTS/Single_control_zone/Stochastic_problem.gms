...
File StochD_output_req/StochD_output_req.txt/;
put StochD_output_req;
Set       alpha   'set of wind penetration levels'    /alpha0*alpha23/;
Parameter Alpha_increment(alpha);
Parameter Penetration_level(alpha);
Parameter Total_expected_costs_for_diff_alpha(alpha);

Loop(alpha,
         Alpha_increment(alpha) = (ord(alpha)-1)*10;
         P_a(k,t,s)=P_b(s,t)*Alpha_increment(alpha);
         ...
         loop((t,i),
                  put Penetration_level(alpha), ord(t), DD_RU.l(t), DD_RD.l(t), R_U.l(i,t), R_D.l(i,t)/;
         );
);
